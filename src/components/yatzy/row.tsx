...
export default function Row(row: rowProps) {
    return <View style={[yatzyStyle.row, { backgroundColor: row.backgroundColor }]} key={row.key}>
        <Text style={[yatzyStyle.head, { fontWeight: 'light' }]}>{row.GameState.score.name}</Text>
        {
            row.GameState.PlayerScore.sort(sortPlayerScoresByPlayersOrder).map((element) => {
                const cellClick = () => {
                    row.onPress(element, row.GameState.score)
                }
                return <TouchableOpacity key={element.player.playerId} style={[{ alignItems: 'center', paddingTop: 5 }, yatzyStyle.cell, element.isRemoved ? row.removedCellStyle : element.score ? row.doneCellStyle : {}]} onPress={cellClick}>
                    {element.isRemoved ?
                        <Icon name='close' style={styles.removedIcon}></Icon>
                        :
                        <Text style={[yatzyStyle.text, sharedStyle.darkFontColor, { fontWeight: 'bold' }]} >
                            {
