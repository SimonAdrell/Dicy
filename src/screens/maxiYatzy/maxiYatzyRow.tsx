    ...
    return <View style={[styles.row, { backgroundColor: row.backgroundColor }]} key={row.key}>
        <Text style={styles.head}>{row.GameState.name}</Text>
        {
            row.GameState.PlayerScore.sort((a, b) => a.playerId - b.playerId).map((element) => {
                ...
                return <TouchableOpacity key={element.playerId} style={styles.cell} onPress={cellClick}>
                    <Text key={element.playerId.toLocaleString()} style={[styles.text, element.isRemoved ? row.removedCellStyle : element.score ? row.doneCellStyle : {}]} >
                        {
